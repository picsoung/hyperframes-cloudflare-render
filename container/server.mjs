...
function runRender(compDir, outFile) {
  return new Promise((resolveRun, reject) => {
    const args = ["render", compDir, "-o", outFile, "--workers", "2", "--quality", "draft"];
    console.log(`[render] spawning: ${HYPERFRAMES_BIN} ${args.join(" ")}`);
...
async function handleRender(req, res) {
  const t0 = Date.now();
  const stepLog = (msg) => console.log(`[render +${Date.now() - t0}ms] ${msg}`);
  stepLog("received POST /render");
  ...
  try {
    ...
    stepLog(`read ${raw.length} bytes from request body`);
    const body = JSON.parse(raw.toString("utf8"));
    if (!Array.isArray(body?.files) || body.files.length === 0) {
      throw new Error("body.files must be a non-empty array");
    }
    await writeFiles(compDir, body.files);
    stepLog(`wrote ${body.files.length} files to ${compDir}`);

    await runRender(compDir, outFile);
    stepLog("hyperframes render finished");
    const { size } = await stat(outFile);
    stepLog(`MP4 size = ${size} bytes`);
    ...
    createReadStream(outFile)
      ...
      .pipe(res)
      .on("close", () => {
        stepLog("MP4 stream closed; cleaning up workdir");
        rm(workRoot, { recursive: true, force: true }).catch(() => {});
      });
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    console.error(`[render +${Date.now() - t0}ms] FAILED: ${message}`);
